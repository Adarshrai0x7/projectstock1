...
def build_knowledge_retriever():
    ...
    try:
        ...
        _rag_retriever = vstore.as_retriever(search_kwargs={"k": settings.rag_top_k})
...
async def get_wikipedia_summary(company_name: str) -> str:
    try:
        import urllib.parse
        import aiohttp
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers={"User-Agent": "FBOT/1.0"}, timeout=5) as r:
                if r.status == 200:
                    data = await r.json()
